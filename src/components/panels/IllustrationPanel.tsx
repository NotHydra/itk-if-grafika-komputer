...
export function IllustrationPanel() {
  ...
  const selectedDef = selectedShape ? ShapeRegistry.get(selectedShape.type) : null;
        ...
        <div className="grid grid-cols-3 gap-2">
          {availableShapes.map((def) => (
            ...
            >
              ...
              <span className="text-[10px] truncate w-full px-1">{def.label}</span>
            </Button>
          ))}
        </div>
      </div>
              ...
              {shapes.map((shape) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => setSelectedObjectId(shape.id)}
                  >
                    <div className="text-muted-foreground shrink-0">
                      {def ? renderIcon(def.icon, "h-3.5 w-3.5") : <Plus className="h-3.5 w-3.5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] text-muted-foreground">
                        Pos: ({shape.x}, {shape.y}) | Rot: {shape.rotation}°
                      </div>
                    </div>
                    <Button
                      variant="ghost"
        ...
        <ScrollArea className="flex-1">
          {selectedShape && selectedDef ? (
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span className="text-[10px] font-mono text-muted-foreground">{selectedShape.rotation}</span>
                  </div>
                  <Slider
                    value={[selectedShape.rotation]}
                    onValueChange={([v]) => updateShape(selectedShape.id, { rotation: v })}
                    min={0}
                    max={360}
                    step={1}
                    className="py-1"
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between">
                    ...
                    <span className="text-[10px] font-mono text-muted-foreground">{selectedShape.scale}x</span>
                  </div>
                  <Slider
                    value={[selectedShape.scale]}
                    onValueChange={([v]) => updateShape(selectedShape.id, { scale: v })}
                    min={0.1}
                    max={3}
                    step={0.1}
                    className="py-1"
                ...
                <div className="space-y-1.5">
                  <Label className="text-[11px] text-muted-foreground">Base Color</Label>
              ...
              {/* Dynamic Meta Fields */}
              {selectedDef.inspectorFields && selectedDef.inspectorFields.length > 0 && (
                <div className="space-y-3">
                  <h4 className="text-[10px] font-bold text-muted-foreground uppercase italic tracking-tight">
                    Properties
                  </h4>
                  {selectedDef.inspectorFields.map((field) => (
                    <div key={field.key} className="space-y-1.5">
                      ...
                      {field.type === 'range' && (
                        <Slider
                          value={[(selectedShape.meta?.[field.key] as number) ?? (field.defaultValue as number)]}
                          onValueChange={([v]) => 
                            updateShape(selectedShape.id, { 
                              meta: { ...selectedShape.meta, [field.key]: v } 
                            })
                          }
                          min={field.min ?? 0}
                          max={field.max ?? 100}
                      ...
                      {field.type === 'number' && (
                        <Input
                          ...
                          onChange={(e) => 
                            updateShape(selectedShape.id, { 
                              meta: { ...selectedShape.meta, [field.key]: parseFloat(e.target.value) || 0 } 
                            })
                          }
                          className="h-7 text-xs"
                        />
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground italic">
                Select a shape from the list to inspect properties
              </p>
            </div>
          )}
        </ScrollArea>

...
export const createPointsSlice: StateCreator<PointsSlice, [], [], PointsSlice> = (set) => ({
  points: [],
  addPoint: (x, y, label, color) =>
    set((s) => ({
      points: [...s.points, { id: nanoid(), x, y, label, color, size: undefined }],
    })),
  removePoint: (id) =>
    set((s) => ({ points: s.points.filter((p) => p.id !== id) })),
  updatePoint: (id, changes) =>
    set((s) => ({
      points: s.points.map((p) => (p.id === id ? { ...p, ...changes } : p)),
    })),
});

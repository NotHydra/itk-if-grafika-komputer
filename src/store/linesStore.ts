...
export const createLinesSlice: StateCreator<LinesSlice, [], [], LinesSlice> = (
	set,
) => ({
	lines: [],
	graphs: [],
	...
	addGraph: (expression, xMin, xMax, sampleCount = 200, color) =>
		set((s) => ({
			graphs: [
				...s.graphs,
				{ id: nanoid(), expression, xMin, xMax, sampleCount, color },
			],
		})),
	removeGraph: (id) =>

...
export const createOpticsSlice: StateCreator<
	OpticsSlice,
	[],
	[],
	OpticsSlice
> = (set) => ({
	...
	removeOpticsObject: (id) =>
		set((s) => ({
			opticsObjects: s.opticsObjects.filter((o) => o.id !== id),
		})),
	updateOpticsObject: (id, changes) =>
		set((s) => ({
			opticsObjects: s.opticsObjects.map((o) =>
				o.id === id ? { ...o, ...changes } : o,
			),
		})),
	...
	removeLightSource: (id) =>
		set((s) => ({
			lightSources: s.lightSources.filter((ls) => ls.id !== id),
		})),
	updateLightSource: (id, changes) =>
		set((s) => ({
			lightSources: s.lightSources.map((ls) =>
				ls.id === id ? { ...ls, ...changes } : ls,
			),
		})),

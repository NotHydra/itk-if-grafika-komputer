...
class Ball {
    balPos = 0;
    balFlo = 400; // Initial floor position (relative to ground)
    axis = 0; // Rotation axis for skeleton
    skel = 0; // Skeleton deformation based on drop height
    velocity = 300;
...
class Renderer {
    ...
    _circlePoints(cx, cy, x, y, buf, W, col) {
        const set = (px, py) => {
    ...
    _ddaLine(x0, y0, xn, yn, buf, W, col) {
        const setP = (px, py) => {
    ...
    renderBall(ball) {
        ...
        const BLUE = 0x0000ff;
        const RED = 0xff0000;
        const drawThick = (ax, ay, bx, by, col) => {
        ...
        for (let py = 0; py < drawDia; py++) {
            for (let px = 0; px < drawDia; px++) {
                ...
                this.putPixel(screenX + px, screenY + py, loData.data[si], loData.data[si + 1], loData.data[si + 2], a);
            }
        }
    }
}
class PhysicsEngine {
    ball;
    GROUND;
    MIN_X = 0; // Left boundary for rolling
    MAX_X = 700; // Right boundary for rolling
    dropActive = false;
    gravityAccum = 0;
    isDrop = true;
    constructor(ball, groundY) {
        this.ball = ball;
        this.GROUND = groundY;
    }
...
class UIController {
    ...
    rollId = 0;
    dropId = 0;
    constructor(ball, phys, rend) {
        this.ball = ball;
        this.phys = phys;
        this.rend = rend;
        this.sliderX = document.getElementById("sliderX");
        this.sliderY = document.getElementById("sliderY");
        this.velInput = document.getElementById("velocityInput");
    ...
    _syncX() {
        this.sliderX.value = String(Math.max(0, Math.min(700, this.ball.balPos)));
